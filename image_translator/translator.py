...
class Translator():
    def __init__(self, translator: str, dest_lang: str, src_lang: str):
    ...
    def translate(self, text: str) -> str:
        if self.translator == 'google':
            return self._run_google(text, self.dest_lang, self.src_lang)
        elif self.translator == 'bing':
            return self._run_bing(text, self.dest_lang, self.src_lang)
        elif self.translator == 'deepl':
            return self._run_deepl(text, self.dest_lang, self.src_lang)

    def translate_paragraph(self, paragraph: Paragraph) -> Paragraph:
        """
        Run translator between Google, Bing and DeepL
        """
        paragraph['translated_text'] = self.translate(paragraph['text'])
        return paragraph

    def _run_google(self, text: str, dest_lang: str, src_lang: str) -> str:
        """
        Run google translator
        """
        tra = Google()

        text = tra.translate(text, dest_lang, src_lang).text

        return text

    def _run_bing(self, text: str, dest_lang: str, src_lang: str) -> str:
        """
        Run bing translator
        """
        tra = Bing()
        text = tra.translate(text, src_lang, dest_lang)

        return text

    def _run_deepl(self, text: str, dest_lang: str, src_lang: str) -> str:
        """
        Run deepl translator
        """
        tra = DeepL(src_lang, dest_lang)
        text = tra.translate(text)
